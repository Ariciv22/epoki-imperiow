# Epoki Imperiów

Strategiczna gra planszowa inspirowana Civilization V i 7 Wonders.

Projekt zawiera zasady gry, opis planszy heksowej, cywilizacje, liderów, technologie, żetony odkryć, cuda, polityki, handel i system wojny.

Na tym etapie religia jest pominięta.

## Dokumentacja projektowa

Pierwsze moduły projektu:

- [00 — Kontekst gry](docs/00-kontekst-gry.md)
- [01 — Wygląd planszy i kafli mapy](docs/01-wyglad-planszy.md)
- [02 — Mapa i heksy](docs/02-mapa-i-heksy.md)
- [03 — Kafle mapy](docs/03-kafle-mapy.md)
- [04 — Żetony odkryć](docs/04-zetony-odkryc.md)
- [05 — Standard wizualny kafli i mapy](docs/05-standard-wizualny-kafli-i-mapy.md)
- [06 — Pierwsze 12 kafli bazowych](docs/06-pierwsze-12-kafli-bazowych.md)
- [07 — Layout kafla](docs/07-layout-kafla.md)
- [08 — Szkielet mapy kontynentalnej](docs/08-szkielet-mapy-kontynentalnej.md)
- [09 — Reguły generowania szkieletu mapy](docs/09-reguly-generowania-szkieletu-mapy.md)

## Prototypy

- [Generator szkieletu mapy](prototypes/map-generator/README.md)

## Referencje wizualne

- [Mapa 04 — szkielet kontynentu](assets/reference/mapa-04-szkielet-kontynentu.svg)

## Główne założenia mapy

- 40 kafli mapy.
- 7 heksów na kafel.
- Łącznie 280 heksów.
- Nieregularna mapa z oceanem na obrzeżach i lądem w środku.
- Zakryte żetony odkryć poza strefami startowymi.
- Brak żetonów odkryć w promieniu do 2 heksów od startu gracza.
